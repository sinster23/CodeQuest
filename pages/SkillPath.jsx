...
const SkillsPathPage = () => {
  ...
  // Add this function to SkillsPathPage component
...
// Update the useEffect that handles challenge completion
useEffect(() => {
  if (!showChallengePage && selectedChallengeNode) {
    // Force refresh when returning from challenge page
    forceRefreshUserData();
  }
}, [showChallengePage, selectedChallengeNode, user]);
...
// Enhanced handleChallengeComplete function for SkillsPathPage.jsx

// Enhanced handleChallengeComplete function for SkillsPathPage.jsx

// Replace the existing handleChallengeComplete function
const handleChallengeComplete = async (nodeId) => {
  ...
  try {
    ...
    const nodeXP = selectedChallengeNode.xp;
    const completionBonus = 10;
    const totalXPGained = nodeXP + completionBonus;
    const currentLanguageProgress = currentUserData.skillsPath[selectedLanguage];
    
    // Check if node is already completed
    if (currentLanguageProgress.completedNodes.includes(nodeId)) {
      console.log('Node already completed, no XP awarded');
      setShowChallengePage(false);
      setSelectedChallengeNode(null);
      return;
    }

    // Update completed nodes and XP
    const newCompletedNodes = [...currentLanguageProgress.completedNodes, nodeId];
    const newLanguageXP = currentLanguageProgress.currentXP + totalXPGained;
    
    const updatedSkillsPath = {
      ...currentUserData.skillsPath,
      [selectedLanguage]: {
        ...currentLanguageProgress,
        completedNodes: newCompletedNodes,
        currentXP: newLanguageXP,
        lastPlayed: serverTimestamp()
      }
    };
    
    // Calculate total XP across all languages
    const totalSkillsPathXP = Object.values(updatedSkillsPath).reduce(
      (total, lang) => total + (lang.currentXP || 0), 0
    );
    
    const newLevel = Math.floor(totalSkillsPathXP / 1000) + 1;
    const newTotalXP = (currentUserData.xp || 0) + totalXPGained;

    // Check for new achievements
    const completedNodesCount = newCompletedNodes.length;
    const languageUpdates = {};
    
    // Language-specific achievements
    if (selectedLanguage === 'javascript') {
      languageUpdates['codeBattles.jsbadge1'] = completedNodesCount >= 1;
      languageUpdates['codeBattles.jsbadge2'] = completedNodesCount >= 5;
      languageUpdates['codeBattles.jsbadge3'] = completedNodesCount >= 10;
      languageUpdates['codeBattles.jsbadge4'] = completedNodesCount >= 15;
      languageUpdates['codeBattles.jsbadge5'] = newLanguageXP >= 2000;
    } else if (selectedLanguage === 'python') {
      languageUpdates['codeBattles.pythonbadge1'] = completedNodesCount >= 1;
      languageUpdates['codeBattles.pythonbadge2'] = completedNodesCount >= 5;
      languageUpdates['codeBattles.pythonbadge3'] = completedNodesCount >= 10;
      languageUpdates['codeBattles.pythonbadge4'] = completedNodesCount >= 12;
      languageUpdates['codeBattles.pythonbadge5'] = newLanguageXP >= 2000;
    }
    // Add similar logic for other languages...

    // Update rank based on total XP
    let rank = "Beginner";
    if (newTotalXP >= 10000) rank = "Legend";
    else if (newTotalXP >= 5000) rank = "Expert";
    else if (newTotalXP >= 2500) rank = "Advanced";
    else if (newTotalXP >= 1000) rank = "Intermediate";

    // Prepare Firestore update
    const updateData = {
      skillsPath: updatedSkillsPath,
      xp: newTotalXP,
      level: newLevel,
      rank: rank,
      badges: (currentUserData.badges || 0) + Object.values(languageUpdates).filter(Boolean).length,
      lastUpdated: serverTimestamp(),
      ...languageUpdates
    };
    ...
    setUserProgress(updatedSkillsPath);
    
    console.log(`Node ${nodeId} completed! +${totalXPGained} XP earned`);
    
    // Close challenge page and return to skills tree
    setShowChallengePage(false);
    setSelectedChallengeNode(null);
...
// Enhanced version with achievement notifications
const handleChallengeCompleteWithNotifications = async (nodeId) => {
  if (!user) return;
  ...
  try {
    ...
    const nodeXP = selectedChallengeNode.xp;
    const completionBonus = 10; // Fixed 10 XP bonus for completing any node
    const totalXPGained = nodeXP + completionBonus;
    const currentLanguageProgress = currentUserData.skillsPath[selectedLanguage];
    ...
    const achievementChecks = {
      javascript: [
        { badge: 'jsbadge1', requirement: 1, name: 'JS Starter' },
        { badge: 'jsbadge2', requirement: 5, name: 'JS Explorer' },
        { badge: 'jsbadge3', requirement: 10, name: 'JS Practitioner' },
        { badge: 'jsbadge4', requirement: 15, name: 'JS Expert' },
        { badge: 'jsbadge5', requirement: 2000, name: 'JS Master', checkXP: true }
      ],
      python: [
        { badge: 'pythonbadge1', requirement: 1, name: 'Python Starter' },
        { badge: 'pythonbadge2', requirement: 5, name: 'Python Explorer' },
        { badge: 'pythonbadge3', requirement: 10, name: 'Python Practitioner' },
        { badge: 'pythonbadge4', requirement: 12, name: 'Python Expert' },
        { badge: 'pythonbadge5', requirement: 2000, name: 'Python Master', checkXP: true }
      ]
      // Add more languages as needed
    };

    if (achievementChecks[selectedLanguage]) {
      achievementChecks[selectedLanguage].forEach(achievement => {
        const currentValue = achievement.checkXP ? newLanguageXP : completedNodesCount;
        const hadAchievement = currentUserData.codeBattles?.[achievement.badge] || false;
        
        if (!hadAchievement && currentValue >= achievement.requirement) {
          languageUpdates[`codeBattles.${achievement.badge}`] = true;
    ...
    const updateData = {
      ...
      level: newLevel,
      rank: rank,
      badges: (currentUserData.badges || 0) + achievements.length,
      lastUpdated: serverTimestamp(),
      ...languageUpdates
    };
    ...
    if (achievements.length > 0) {
      ...
      // Example: setShowAchievementModal(achievements);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-black text-white relative overflow-hidden">
      ...
      <div
        className="floating-orb absolute w-4 h-4 bg-purple-400 rounded-full"
        style={{ top: "10%", left: "5%", animationDelay: "0s" }}
      ></div>
      <div
        className="floating-orb absolute w-3 h-3 bg-cyan-400 rounded-full"
        style={{ top: "70%", right: "8%", animationDelay: "2s" }}
      ></div>
      <div
        className="floating-orb absolute w-2 h-2 bg-yellow-400 rounded-full"
        style={{ top: "30%", right: "15%", animationDelay: "4s" }}
      ></div>
      ...
      <div className="container mx-auto px-6 py-8 relative z-10">
            ...
            <div className="bg-black/50 p-3 rounded border border-purple-400/30">
              ...
              <div className="pixel-font text-lg text-white font-bold">
                {Object.values(userProgress).reduce(
                  (total, lang) => total + lang.currentXP,
                  0
                )}
              </div>
            </div>
                ...
                className={`retro-card p-4 cursor-pointer transition-all duration-300 transform hover:scale-105
                  ${
                    selectedLanguage === lang.id
                      ? "border-purple-400 shadow-purple-400/50 shadow-lg"
                      : "hover:border-white/40"
                  }`}
                onClick={() => setSelectedLanguage(lang.id)}
              >
                <div
                  className={`w-12 h-12 bg-gradient-to-br ${lang.color} rounded-full flex items-center justify-center mx-auto mb-3`}
                >
                  <Icon className="w-6 h-6 text-white" />
                </div>

                <h3 className="pixel-font text-xs text-center text-white mb-2">
                  {lang.name}
                </h3>
                ...
                <div className="text-center">
                  <span className="pixel-font text-xs text-gray-300">
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="pixel-font text-xs text-white font-bold">
                      {userProgress[selectedLanguage].completedNodes.length}/
                      {skillTrees[selectedLanguage].nodes.length}
                    </span>
                  </div>
                  <div className="w-full bg-gray-700 h-3 rounded">
                    <div
                      ...
                      style={{
                        width: `${
                          (userProgress[selectedLanguage].completedNodes
                            .length /
                            skillTrees[selectedLanguage].nodes.length) *
                          100
                        }%`,
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-center">
                    <div className="pixel-font text-lg text-yellow-400 font-bold">
                      {skillTrees[selectedLanguage].nodes.reduce(
                        (total, node) =>
                          userProgress[
                            selectedLanguage
                          ].completedNodes.includes(node.id)
            ...
            {selectedNode && (
              <div className="retro-card p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="pixel-font text-lg font-bold text-yellow-400">
                    {selectedNode.name}
                  </h3>
                  <button
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="pixel-font text-xs text-gray-400">
                      Difficulty:
                    </span>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="pixel-font text-xs text-purple-400 font-bold">
                      +{selectedNode.xp} XP
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="pixel-font text-xs text-cyan-400 font-bold">
                      {selectedNode.challenges} problems
                    </span>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {selectedNode.prerequisites.map((prereq) => {
                          const prereqNode = skillTrees[
                            selectedLanguage
                          ].nodes.find((n) => n.id === prereq);
                          const isCompleted =
                            userProgress[
                              selectedLanguage
                      ...
                      if (status.completed) {
                        ...
                        return (
                          <div className="text-center">
                            ...
                            <div className="pixel-font text-xs text-blue-400 mb-2">
                              READY TO START
                            </div>
                            <button
                        ...
                        return (
                          <div className="text-center">
                            ...
                            <div className="pixel-font text-xs text-gray-500 mb-2">
                              LOCKED
                            </div>
        ...
        <div className="mt-8 retro-card p-6">
          <h3 className="pixel-font text-lg font-bold text-white mb-4 text-center">
            Legend
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <div className="pixel-font text-sm text-white font-bold">
                  Completed
                </div>
                <div className="pixel-font text-xs text-gray-400">
